      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.nav
            ...
            className="z-50 rounded-2xl transition-all duration-500 ease-in-out"
          >
            <div className="max-w-7xl mx-auto px-6 md:px-8">
              <div className="flex items-center justify-between h-16 md:h-20">
                ...
                <div className='flex items-center space-x-4 w-full justify-between'>
                  ...
                  {/* Logo */}
                  <Link href="/" className="text-2xl font-black text-neutral-900 dark:text-[white] tracking-tight">
                    RekaKarya<span className="text-neutral-400 dark:text-neutral-500">.</span>
                  </Link>

                  <ul className='flex space-x-2 shadow-md hidden md:flex text-sm font-medium text-neutral-600 dark:text-neutral-300 pt-4 pb-4 pr-2 pl-2 rounded-xl bg-[#f0f0f0] dark:bg-neutral-800'>
                    <li><a href="#" className='pt-3 pb-3 pr-5 pl-5 hover:bg-white rounded-lg font-bold hover:shadow-sm hover:text-[#626262] text-[#b2b2b2]'>Home</a></li>
                    <li><a href="#" className='pt-3 pb-3 pr-5 pl-5 hover:bg-white rounded-lg font-bold hover:shadow-sm hover:text-[#626262] text-[#b2b2b2]'>About us</a></li>
                    <li><a href="#" className='pt-3 pb-3 pr-5 pl-5 hover:bg-white rounded-lg font-bold hover:shadow-sm hover:text-[#626262] text-[#b2b2b2]'>Services</a></li>
                    <li><a href="#" className='pt-3 pb-3 pr-5 pl-5 hover:bg-white rounded-lg font-bold hover:shadow-sm hover:text-[#626262] text-[#b2b2b2]'>Blog</a></li>
                  </ul>
                  ...
                  <div className='flex items-center space-x-3'>
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className='hidden lg:inline-flex items-center space-x-2 pt-3 pb-3 pr-5 pl-5 dark:bg-gradient-to-b dark:from-neutral-100 dark:to-neutral-300 bg-gradient-to-b from-white to-gray-100 rounded-xl shadow-md font-semibold text-md text-[#626262] transition-all duration-200'
                      aria-label='LLL'>Learn More</motion.button>
                    {/* Menu Button */}
                    <motion.button
                      onClick={() => setIsOpen(!isOpen)}
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      className="relative p-3 bg-gradient-to-t from-neutral-900 to-neutral-600 dark:bg-gradient-to-t dark:from-neutral-200 dark:to-neutral-400 text-white dark:text-neutral-900 rounded-xl shadow-lg hover:bg-neutral-800 dark:hover:bg-neutral-100 transition-colors duration-300"
                    ...
                    {navItems.map((item, index) => (
                      ...
                      >
                        <Link
                          href={item.path}
                          onClick={() => setIsOpen(false)}
                          className={`group flex items-center justify-between py-4 px-6 rounded-2xl border-3 border-transparent hover:border-white/30 transition-all duration-300 ${pathname === item.path || (pathname.startsWith(item.path) && item.path !== '/')
                            ? 'bg-white text-neutral-900'
                            : 'text-white hover:bg-white/10'
                            }`}
                        >
                          <div className="flex items-center space-x-4">
                            <span className={`text-sm font-mono ${pathname === item.path || (pathname.startsWith(item.path) && item.path !== '/') ? 'text-neutral-500' : 'text-white/50'
                              }`}>
                              {item.number}
                            </span>
                    ...
                    {navItems.map((item, index) => (
                      ...
                      >
                        <Link
                          href={item.path}
                          onClick={() => setIsOpen(false)}
                          className={`group flex items-center justify-between py-3 px-4 rounded-xl border-3 border-transparent hover:border-white/30 transition-all duration-300 ${pathname === item.path || (pathname.startsWith(item.path) && item.path !== '/')
                            ? 'bg-white text-neutral-900'
                            : 'text-white hover:bg-white/10'
                            }`}
                        >
                          <div className="flex items-center space-x-3">
                            <span className={`text-xs font-mono ${pathname === item.path || (pathname.startsWith(item.path) && item.path !== '/') ? 'text-neutral-500' : 'text-white/50'
                              }`}>
                              {item.number}
                            </span>
